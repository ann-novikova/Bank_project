...
def search_string(transactions_list: list[dict], search_string: str) -> list[dict]:
    return [transaction for transaction in transactions_list
...
def count_by_category(transactions_list: list[dict], category_list: list) -> dict:
    count_description = dict(Counter([transaction['description'] for transaction in transactions_list
...
print(count_by_category(transaction_for_generators, [
    'Перевод организации', 'Перевод с карты на карту', 'Отмена']))